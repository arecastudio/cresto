...
FROM pesanan_detail
WHERE
	batal='TIDAK' AND pesanan_id IN (SELECT id FROM pesanan WHERE status='BUKA'
	AND kirim='SDH'
...
if (isset($_POST['submit']) || isset($_POST['cetak'])) {
	...
	FROM pesanan_detail
	WHERE batal='TIDAK' AND pesanan_id IN (SELECT id FROM pesanan WHERE status='BUKA' AND kirim='SDH' AND meja_nomor='$nomor')
		...
		if (isset($_POST['submit'])) {
			...
			if ($terima>=$gTotal) {
				...
				if ($rs=$con->query("SELECT id FROM pesanan WHERE meja_nomor='$nomor';")) {
					...
					$con->query("INSERT IGNORE INTO faktur_stat(faktur_id)VALUES($no_faktur);");
				}

				$con->query("UPDATE pesanan SET status='TUTUP',waktu_lunas=current_timestamp(),uang_diterima=$terima,uang_kembali=$kembali WHERE meja_nomor='$nomor';");
