...
if(isset($_GET['item'])){
	...
	if($meja=$con->query("SELECT id FROM pesanan WHERE status='BUKA' AND meja_nomor='$nomor' AND kirim='BLM';")){
		if ($row=$meja->fetch_row()) {
			...
			//echo $pesanan_id."\n\n";
			$sql_input="
			INSERT IGNORE INTO pesanan_detail(pesanan_id,produk_id,produk_nama,produk_harga,kategori_id,jumlah,operator,siap)
			VALUES($pesanan_id,$produk_id,(SELECT DISTINCT nama FROM produk WHERE id=$produk_id),(SELECT DISTINCT harga FROM produk WHERE id=$produk_id),(SELECT DISTINCT kategori_id FROM produk WHERE id=$produk_id),1,'$user_name','$siap')
			;";
			$con->query($sql_input);
		}
	}
...
if($meja=$con->query($sql_menu)){
	while ($row=$meja->fetch_row()) {
		...
		echo"
		<a href=\"?ref=data-pesan-meja-langsung&nomor=$nomor&item=$row[0]\">
...
if (isset($_POST['submit'])) {
	...
	header('location: ?ref=data-pesan-meja-langsung&nomor='.$nomor);//exit();
}
...
<tr>
	...
	<td>".substr($row[5], 0,1)."</td>
<td>
<a href=\"?ref=data-pesan-meja-langsung&nomor=$nomor&item_ubah=$row[1]&item_nama=$row[2]&item_jumlah=$row[4]&item_bungkus=$row[5]\"><img src=\"images/edit.png\" width=\"16px\" title=\"Edit Item ini\"></a>&nbsp;&nbsp;&nbsp;
<a href=\"?ref=data-pesan-meja-langsung&nomor=$nomor&del=$row[1]&pesanan_id=$row[0]&item_bungkus=$row[5]\"><img src=\"images/delete.png\" width=\"16px\" title=\"Hapus Item ini\" onclick=\"return confirm('Yakin untuk hapus?');\"></a>
</td>
</tr>
		";
	}
