<?php
ob_start();
lock_page($con,"master_daftar_menu_tambahan",$user_name,$keys);
?>

<div class="lokasi">
	<a href="?ref=master">Master</a> &rarr; Daftar Menu Tambahan
</div>

<div class="judul-frame">
	<b>Master Daftar Menu Tambahan</b>
</div>
...
<div class="data-view">
	<table width="700px" cellspacing="3" cellpadding="5" border="1">
		...
		if (isset($_GET['del'])) {
			...
			header('location: ?ref=menu-tambahan');exit;
		}
		...
		if(isset($_POST['submit'])){
			...
			if ( strlen(trim($nama))>0 && strlen(trim($harga))>0 ) {
				...
				$qry="INSERT INTO produk(id,nama,harga,gambar,kategori_id,waktu_ubah,operator,tambahan)VALUES($id,'$nama',$harga,'$pic_name',$kategori_id,current_timestamp(),'$user_name','YA') ON DUPLICATE KEY UPDATE nama='$nama',harga=$harga,gambar='$pic_name',kategori_id=$kategori_id,waktu_ubah=current_timestamp(),operator='$user_name',tambahan='YA';";
				if($id==null) $qry="INSERT INTO produk(nama,harga,gambar,kategori_id,operator,tambahan)VALUES('$nama',$harga,'$pic_name',$kategori_id,'$user_name','YA');";
				$con->query($qry);
		...
		if($meja=$con->query("SELECT DISTINCT id,nama,(SELECT DISTINCT kategori FROM kategori WHERE id=produk.kategori_id)AS kat,harga,SUBSTR(gambar,1,10) AS gbr,kategori_id FROM produk WHERE tambahan='YA' ORDER BY id ASC;")){
			while ($row=$meja->fetch_row()) {
				...
				<tr>
					...
					<td>$row[4]</td>
					<td>
						<a href=\"?ref=menu-tambahan&pil=$row[0]&produk_nama=$row[1]&produk_kategori=$row[5]&produk_harga=$row[3]&produk_gambar=$row[4]\"><img src=\"images/edit.png\" width=\"16px\" title=\"Edit Meja ini\"></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						<a href=\"?ref=menu-tambahan&del=$row[0]\"><img src=\"images/delete.png\" width=\"16px\" title=\"Hapus Menu ini\" onclick=\"return confirm('Yakin untuk hapus?');\"></a>
					</td>
				</tr>
				";
			}
